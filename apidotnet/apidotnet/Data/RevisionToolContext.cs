...
public partial class RevisionToolContext : DbContext
{
    ...
    public int NextPointInModule(int p_USER_ID, int MODULE_ID)
    {
        using (var command = Database.GetDbConnection().CreateCommand())
        {
            command.CommandText = "Next_Point_IN_MODULE";
            command.CommandType = CommandType.StoredProcedure;

            var p1 = new MySqlParameter("@p_USER_ID", p_USER_ID);
            var p2 = new MySqlParameter("@p_MODULE_ID", MODULE_ID);
    ...
    public int NextPoint(int userId)
    {
        using (var command = Database.GetDbConnection().CreateCommand())
        {
            command.CommandText = "Next_Point"; // Replace with your stored procedure name
            command.CommandType = CommandType.StoredProcedure;

            var p1 = new MySqlParameter("@p_USER_ID", userId);
            var resultParam = new MySqlParameter
            {
                ParameterName = "@p_POINT_ID",
